...
def regression_metrics(output_node_shape, output_node, variables=None):
    """
    [Int] -> tf.Tensor -> [tf.Variable]? -> (TargetNode, LossNode, Optimiser)
    Create metrics - target node, loss node, and optimiser - for a
    regression model.
    """
    node_name = output_node.name
    target_node = placeholder_node(node_name + '.target', output_node_shape,
        dynamic_dimensions=1)
    loss_node = tf.losses.mean_squared_error(target_node, output_node)
    optimiser = default_adam_optimiser(loss_node, variables=variables)
    return target_node, loss_node, optimiser


def classification_metrics(output_node_shape, output_node, variables=None):
    """
    [Int] -> tf.Tensor -> [tf.Variable]? ->
...
def beta_variational_metrics(input_node, reconstruction_node, means_node,
        stddevs_node, beta=1.0, variables=None, eps=1e-7):
    """
    tf.Tensor -> tf.Tensor -> tf.Tensor -> tf.Tensor -> Float? ->
    ...
    reconstruction_loss, _ = reconstruction_metrics(input_node,
        reconstruction_node, variables=[])
    kl_loss, _ = variational_metrics(means_node, stddevs_node, variables=[])
    composite_loss = reconstruction_loss + beta * kl_loss
    optimiser = default_adam_optimiser(composite_loss, variables=variables)
    return reconstruction_loss, kl_loss, composite_loss, optimiser


def reconstruction_metrics(input_node, reconstruction_node, variables=None):
    """
    tf.Tensor -> tf.Tensor -> [tf.Variable]? -> (LossNode, Optimiser)
...
def variational_metrics(means_node, stddevs_node, variables=None, eps=1e-7):
    """
    tf.Tensor -> tf.Tensor -> [tf.Variable]? -> Float? -> (LossNode, Optimiser)
...
def default_adam_optimiser(loss_node, variables=None):
    """
    LossNode -> Optimiser
    ...
    return tf.train.AdamOptimizer(name=node_name + '.adam_optimiser')   
        .minimize(loss_node, var_list=variables, name=node_name + '.adam_optimiser.minimise')
